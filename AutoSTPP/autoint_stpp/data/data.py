...
class SlidingWindowWrapper(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, seqs, lookback=20, lookahead=1, normalized=False, roll=True, min=None, max=None, device=None):
        # def __init__(self, seqs, lookback=20, lookahead=1, normalized=False, roll=True, min=None, max=None, device=torch.device("cuda:0")):
        """
        ...
        """
        # If no device specified, stay on CPU and let Lightning handle GPU placement
        if device is None:
        ...
        for i, seq in enumerate(self.seqs):
            ...
            #self.seqs[i][:, 0] = torch.diff(seq[:, 0], dim=0, prepend=torch.tensor([0.0], device=seq.device))
