import torch
import numpy as np
from autoint_stpp.models.lightning.stpp import BaseSTPointProcess
from autoint_stpp.integration.autoint import Cuboid, SumNet, ProdNet, act_dict
...
class AutoIntSTPointProcess(BaseSTPointProcess):
    def __init__(self, *args, **kwargs):
        ...
        Constructor now supports:
          - AutoIntSTPointProcess(Config)
          - AutoIntSTPointProcess(n_prodnet=5, hidden_size=64, ...)
        """
        # Case 1: Config or dict passed
        if len(args) == 1 and not kwargs:
            cfg = args[0]
            if hasattr(cfg, "__dataclass_fields__"):
                logger.debug("[AutoIntSTPP] Received Config dataclass — converting to dict.")
                kwargs = asdict(cfg)
            elif isinstance(cfg, dict):
                logger.debug("[AutoIntSTPP] Received plain dict config.")
                kwargs = cfg
            else:
                raise TypeError(f"Unexpected argument type for AutoIntSTPointProcess: {type(cfg)}")

        # Split kwargs into base and model-specific
        base_sig = signature(BaseSTPointProcess.__init__)
        base_keys = set(base_sig.parameters.keys()) - {"self", "args", "kwargs"}
        base_kwargs = {k: v for k, v in kwargs.items() if k in base_keys}
        model_kwargs = {k: v for k, v in kwargs.items() if k not in base_keys}

        if model_kwargs:
            logger.debug(f"[AutoIntSTPP] Model-specific keys detected: {list(model_kwargs.keys())}")

        # Init BaseSTPointProcess
        super().__init__(**base_kwargs)

        # Save all hyperparameters (including bias)
        self.hparams.update(model_kwargs)

        # Default values if missing
        self.hparams.setdefault("bias", True)
        self.hparams.setdefault("n_prodnet", 2)
        self.hparams.setdefault("hidden_size", 128)
        self.hparams.setdefault("num_layers", 2)
        self.hparams.setdefault("activation", "tanh")
        self.hparams.setdefault("trunc", False)
    ...
    def callbacks(self):
        # Return an empty list if no custom callbacks are needed
        return []
